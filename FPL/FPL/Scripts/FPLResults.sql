IF NOT EXISTS (SELECT * FROM [sys].[schemas] WHERE [name] = 'FPL')
    EXEC('CREATE SCHEMA [FPL]');
GO
IF EXISTS (SELECT * FROM [sys].[tables] WHERE [name] = 'Results')
DROP TABLE [FPL].[Results];
GO
...
INSERT INTO [FPL].[Results] ([Player], [Game], [Season], [Points], [OverallRank])
VALUES
...
WHERE 1 = 1
		...
		AND [Season] <> '2013/14'
GROUP BY [Player]
ORDER BY SUM(ISNULL([Points], 0)) DESC -- ORDER BY [TotalPoints]
GO
